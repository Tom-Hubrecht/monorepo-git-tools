...
pub struct MgtCommandCheck {
    // flags
    #[options(
        help = "if the <repo-file> is a directory, by default mgt only looks for files ending in .rf, but with the --all flag, you are telling mgt to get any file it finds from the <repo-file> directory"
    )]
    pub all: bool,
    ...
    pub recursive: bool,
    #[options(
        help = "check if the remote has commits not present in this local branch. This is the default"
    )]
    pub remote: bool,
    #[options(short = "h")]
    pub help: bool,
    ...
    #[options(
        meta = "BRANCH-NAME",
        help = "check updates to/from a specific local branch instead of the current HEAD"
    )]
    pub local_branch: Option<String>,
    #[options(
        short = "b",
        meta = "BRANCH-NAME",
        help = "check updates to/from a specific remote branch instead of what's in the repo file"
    )]
...
pub struct MgtCommandDifflog {
    ...
    #[options(
        short = "m",
        help = "(fullbase not implemented yet) Valid modes are [topbase, rewind, fullbase]. default is rewind"
    )]
    pub traversal_mode: Option<ABTraversalMode>,

    #[options(
        short = "w",
        help = "Force specify a width to display the log. default is to use whole terminal"
    )]
...
pub struct MgtCommandSplit {
    #[options(
        short = "g",
        long = "gen-repo-file",
        help = "generate a repo file from the provided remote repo and the --as argument gets mapped to [include_as]"
    )]
    ...
    #[options(
        meta = "N",
        help = "when pulling from remote, limit to N commits from the current tip. This is probably only useful the first time you do a split-in"
    )]
    ...
    #[options(
        optional,
        short = "t",
        help = "like --rebase, but it finds a fork point by stopping at the first commit that two branches have in common. This is useful as an 'update' mechanism. Optionally provide a '--topbase BRANCH-NAME' to topbase onto that branch instead of the default."
    )]
    pub topbase: Option<String>,

    #[options(
        long = "as",
        help = "path relative to root of the local repository that will contain the entire repository being split"
    )]
...
pub struct MgtCommandVerify {
    ...
    #[options(
        help = "format the mapping nicely. implies verbose. wont work well on small terminals though"
    )]
    pub pretty: bool,

    #[options(
        help = "provide a list of files to verify from stdin, one file per line. By default we get this list of files for you via:\ngit ls-tree -r HEAD --name-only --full-tree\n You can achieve the default behavior by doing:\n git ls-tree -r HEAD --name-only --full-tree | mgt verify-rf --stdin <PATH/TO/REPOFILE>"
    )]
    pub stdin: bool,
}
...
pub struct MgtCommandSync {
    ...
    #[options(
        help = "when iterating the sync of multiple repo files, if a single one fails, do not sync the rest"
    )]
    pub fail_fast: bool,

    #[options(
        short = "b",
        help = "by default, we fetch the remote HEAD when looking for updates to sync. With --ask-branches you will be asked which remote branch you wish to fetch for each remote url we fetch"
    )]
    pub ask_branches: bool,

    #[options(
        help = "specify a maximum number of attempts when being asked an interactive question. Default is infinite. This is useful only for scripts to prevent them from hanging"
    )]
    pub max_interactive_attempts: Option<usize>,

    #[options(
        help = "only show the summary of the potential sync, but do not perform any filter, pull, or push operations"
    )]
...
pub enum MgtSubcommands {
    ...
    #[options(
        help = "Interactively sync one or more repo files between local and remote repositorie(s)"
    )]
    Sync(MgtCommandSync),

    #[options(
        help = "View a log comparing two branches that have potentially unrelated history using a topbase algorithm"
    )]
    ...
    #[options(
        help = "rebase top branch onto bottom branch but stop the rebase after the first shared commit"
    )]
    Topbase(MgtCommandTopbase),

    #[options(
        help = "fetch and rewrite a remote repository's history onto a new branch according to the repo file rules"
    )]
    SplitIn(MgtCommandSplit),

    #[options(
        help = "fetch and rewrite a remote repository's history onto a new branch and into the --as <subdirectory>"
    )]
    SplitInAs(MgtCommandSplit),

    #[options(
        help = "create a new branch with this repository's history rewritten according to the repo file rules"
    )]
    SplitOut(MgtCommandSplit),

    #[options(
        help = "create a new branch with this repository's history rewritten according to the --as <subdirectory>"
    )]
...
pub fn get_version_str() -> String {
    format!("{} {}", env!("CARGO_PKG_VERSION"), env!("LATEST_COMMIT"),)
}
...
pub fn print_usage<A: AsRef<impl Options>>(
    ...
    println!("USAGE:\n{}{} {}\n", space, cmd_name, usage_line);

    let sub_usage = mgt_opts.as_ref().format_sub_usage_string_with_filters(
        Some(100),
        Some(4),
        Some(4),
        filters,
...
pub fn print_program_usage<A: AsRef<impl Options>>(mgt_opts: A) {
    ...
    println!(
        "{} {}\n{}\n{}\n\nUSAGE:\n{}{} [SUBCOMMAND] [OPTIONS]\n",
        app_name, version_str, author, about, space, app_name
    );
...
pub fn get_cli_input() -> Mgt {
    ...
    let is_help = match cli.command {
        ...
        Some(ref cmd) => match cmd {
            ...
            MgtSubcommands::VerifyRepoFile(v) | MgtSubcommands::VerifyRf(v) => {
                if cli.help || v.help {
        ...
        Some(mut command) => match command {
            ...
            MgtSubcommands::DiffLog(mut cmd) => {
                if cmd.branches.len() != 2 {
                    die!(
                        "Must provide exactly two branches to compare. You provided:\n{:?}",
                        cmd.branches
                    );
                }
                run_difflog(&mut cmd);
            ...
            MgtSubcommands::VerifyRepoFile(ref mut cmd) | MgtSubcommands::VerifyRf(ref mut cmd) => {
                run_verify(cmd);
            }
