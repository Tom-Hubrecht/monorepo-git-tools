use std::{collections::HashSet, fmt::Debug, hash::Hash, io, str::FromStr};
...
use super::git_helpers3::{RawBlobSummary, RawBlobSummaryWithoutPath};
use super::git_helpers3::{RawBlobSummaryEndState, RawBlobSummaryEndStateWithoutPath};
use super::ioerr;
...
pub fn topbase(
    ...
    if num_commits_to_take == 0 {
        if dry_run {
            println!(
                "{}Nothing to topbase. Returning to {}",
                log_p, upstream_branch
            );
    ...
    let args = if num_commits_to_take == num_commits_of_current {
        ...
        println!(
            "{}no commit of {} exists in {}. rebasing non-interactively",
            log_p, current_branch, upstream_branch
        );

        let args = vec!["git".into(), "rebase".into(), upstream_branch.clone()];
        args
    } else {
        vec![]
    };
    ...
    if args.len() != 0 {
        ...
        let err_msg = match exec_helpers::execute(&str_args[..]) {
    ...
    if dry_run || verbose {
        ...
        println!(
            "GIT_SEQUENCE_EDITOR=\"echo $rebase_data >\" git rebase -i --onto {} {}~{} {}",
            upstream_branch, current_branch, num_commits_to_take, current_branch,
        );
        if dry_run {
            return Ok(());
        }
...
pub fn run_topbase(cmd: &mut MgtCommandTopbase) {
    ...
    let res = topbase(top, base, cmd.dry_run, cmd.verbose, should_add_branch_label);
    if let Err(e) = res {
...
impl<'a, T: Debug + Default + From<RawBlobSummary> + Eq + Hash> BranchIterativeCommitLoader<'a, T> {
    ...
    pub fn load_next<F, B>(&mut self, should_use_blob: &mut B, cb: F) -> io::Result<()>
    where
        F: FnMut((&CommitWithBlobs, &HashSet<T>)) -> bool,
        B: FnMut(&mut RawBlobSummary, &str) -> bool,
    {
        ...
        git_helpers3::iterate_blob_log(&use_committish, Some(use_n), |c| {
            ...
            let blob_set: HashSet<T> = before_hash.drain(..).map(|x| T::from(x)).collect();
    ...
    pub fn get_all_above<C: From<CommitWithBlobs>>(&mut self, commit_id: &str) -> Option<Vec<C>> {
...
>(
    ...
    while !a_loader.entirely_loaded || !b_loader.entirely_loaded {
        ...
        if let Some((a_fork, b_fork)) = found_fork_point {
            let top_a_commits =
                a_loader
                    .get_all_above::<C>(&a_fork.commit.id.hash)
                    .ok_or(ioerr!(
                        "Found a fork point {}, but failed to find commits above it?",
                        a_fork.commit.id.short()
                    ))?;
            let successful_topbase = SuccessfulTopbaseResult {
                top_commits: top_a_commits,
                top_right_commits: get_rewind_commits_from_loader(
                    &mut b_loader,
                    &b_fork.commit.id.hash,
                    should_rewind,
                ),
...
>(
    ...
    if let Some(cb) = should_use_blob_cb {
        return find_a_b_difference2_iterative_traversal::<T, C, B>(
            a_committish,
            b_committish,
            traverse_n,
            should_rewind,
    ...
    find_a_b_difference2_iterative_traversal::<T, C, NopCB>(
        a_committish,
        b_committish,
        traverse_n,
        should_rewind,
...
>(
    ...
    if let Some(n) = traverse_n_at_a_time {
        ...
        match hashing_mode {
            BlobHashingMode::Full => {
                find_a_b_difference2_iterative_traversal_opt::<RawBlobSummary, C, _>(
                    a_committish,
                    b_committish,
                    n,
                    should_rewind,
            ...
            BlobHashingMode::WithoutPath => {
                find_a_b_difference2_iterative_traversal_opt::<RawBlobSummaryWithoutPath, C, _>(
                    a_committish,
                    b_committish,
                    n,
                    should_rewind,
            ...
            BlobHashingMode::EndState => {
                find_a_b_difference2_iterative_traversal_opt::<RawBlobSummaryEndState, C, _>(
                    a_committish,
                    b_committish,
                    n,
                    should_rewind,
            ...
            BlobHashingMode::EndStateWithoutPath => find_a_b_difference2_iterative_traversal_opt::<
...
>(
    ...
    git_helpers3::iterate_blob_log(b_committish, None, |c| {
        let b_blob_set = if let Some(ref mut cb) = should_use_blob_cb {
            ...
            let b_blob_set: HashSet<T> = before_hash.drain(..).map(|x| T::from(x)).collect();
            b_blob_set
        } else {
            // otherwise, just hash them all
            let b_blob_set: HashSet<T> = c.blobs.iter().cloned().map(|x| T::from(x)).collect();
            b_blob_set
        };
    ...
    git_helpers3::iterate_blob_log(a_committish, None, |c| {
        ...
        let a_blob_set = if let Some(ref mut cb) = should_use_blob_cb {
            ...
            let a_blob_set: HashSet<T> = before_hash.drain(..).map(|x| T::from(x)).collect();
            a_blob_set
        } else {
            let a_blob_set: HashSet<T> = c.blobs.drain(..).map(|x| T::from(x)).collect();
            a_blob_set
        };
...
pub fn simplest_topbase<C: From<CommitWithBlobs>>(
    ...
    match hashing_mode {
        ...
        BlobHashingMode::WithoutPath => {
            simplest_topbase_inner::<RawBlobSummaryWithoutPath, C, NopCB>(
                a_committish,
                b_committish,
                false,
                None,
        ...
        BlobHashingMode::EndStateWithoutPath => simplest_topbase_inner::<
            RawBlobSummaryEndStateWithoutPath,
            C,
            NopCB,
...
// pub fn rewind_topbase<C: From<CommitWithBlobs>, B: FnMut(&mut RawBlobSummary, &str) -> bool>(
//     a_committish: &str,
//     b_committish: &str,
//     hashing_mode: BlobHashingMode,
//     should_use_blob_cb: Option<B>,
...
//             should_use_blob_cb,
//         ),
//         BlobHashingMode::EndStateWithoutPath => {
//             simplest_topbase_inner::<RawBlobSummaryEndStateWithoutPath, C, B>(
//                 a_committish,
//                 b_committish,
//                 true,
//                 should_use_blob_cb,
